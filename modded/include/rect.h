...
namespace core
{
	...
	*/
	...
	public:
		...
		//! set the rectangle
		template<class U>
		rect<T>& set( vector2d<T> pos, dimension2d<U> size ) {
			UpperLeftCorner = pos;
			LowerRightCorner.set( pos.X + (T)size.Width, pos.Y + (T)size.Height );
			return *this;
		}

		// Added by Nic Anderson
		//! move the entire rectangle by the given shift amounts
		rect<T>& move(T x, T y)
		{
			UpperLeftCorner.move(x,y);
			LowerRightCorner.move(x,y);
			return *this;
		}

		// Added by Nic Anderson
		//! return the difference of two rectangles
		rect<T> delta(const rect<T>& other)
		{
			return rect<T>(*this).makeDelta(other);
		}

		// Added by Nic Anderson
		//! converts the rect into the difference of two rectangles and returns it
		rect<T>& makeDelta(const rect<T>& other)
		{
