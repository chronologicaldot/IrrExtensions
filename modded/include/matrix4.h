...
*/
	...
	*/
	...
	inline bool CMatrix4<T>::getInverse(CMatrix4<T>& out) const
	{
		...
		// Nic has set this to use type T for accuracy
		T d = (m[0] * m[5] - m[1] * m[4]) * (m[10] * m[15] - m[11] * m[14]) -
		...
		if( core::iszero(d) )
			return false;
		// ^ epsilon removed by Nic for accuracy with double.
		// iszero has an override for doubles.
